...
const strongPassword = (p)=> p && p.length>=8 && /[A-Z]/.test(p) && /[a-z]/.test(p) && /[0-9]/.test(p) && /[^A-Za-z0-9]/.test(p);
...
app.post("/api/signup", authLimiter, async (req,res)=>{
  ...
  if(!fullname || !username || !email || !password || !confirm) return res.status(400).json({error:"All fields are required."});
  if(fullname.length<3 || fullname.length>100) return res.status(400).json({error:"Full name must be 3–100 characters."});
  if(!/^[a-zA-Z0-9_]{3,30}$/.test(username)) return res.status(400).json({error:"Username must be 3–30 chars (letters, numbers, underscore)."});
  if(!isEmail(email)) return res.status(400).json({error:"Invalid email address."});
  if(!strongPassword(password)) return res.status(400).json({error:"Password must be 8+ chars and include Uppercase, Lowercase, Number, Symbol."});
  if(password!==confirm) return res.status(400).json({error:"Passwords do not match."});

  const users = readUsers();
  const usernameLower = username.toLowerCase();
  if(users.some(u=>u.usernameLower===usernameLower)) return res.status(409).json({error:"Username already exists."});
  if(users.some(u=>u.email===email)) return res.status(409).json({error:"Email already registered."});

  const hash = await bcrypt.hash(password, 12);
  const user = { id: uuidv4(), fullname, username, usernameLower, email, passwordHash: hash, failedAttempts:0, lockUntil:0, createdAt: new Date().toISOString() };
...
app.post("/api/login", authLimiter, async (req,res)=>{
  ...
  const user = users.find(u=>u.usernameLower===loginLower || u.email===loginLower);
  ...
  if(!ok){
    ...
    if(user.failedAttempts >= 5){ user.lockUntil = now + 5*60*1000; user.failedAttempts = 0; }
    writeUsers(users);
    return generic();
  }

  user.failedAttempts = 0; user.lockUntil = 0; writeUsers(users);
  req.session.user = { id:user.id, username:user.username, fullname:user.fullname, email:user.email };
